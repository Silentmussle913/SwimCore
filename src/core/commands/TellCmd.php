...
class TellCmd extends BaseCommand
{
  ...
  public function onRun(CommandSender $sender, string $aliasUsed, array $args): void
  {
    if (!isset($args["player"]) || !isset($args["message"])) {
      ...
      self::messageLogic($sender, $args);
    }
  }

  // reply command uses this
  public static function messageLogic(CommandSender $sender, array $args): void
  {
    $target = SeeNick::getPlayerFromNick($args["player"]);
    if (!$target) {
    ...
    $target->sendMessage(TF::AQUA . "From " . $sender->getCosmetics()->getNameColor() . $sender->getRank()->rankString() . TF::GRAY . " » " . TF::WHITE . $msg);
    $sender->sendMessage(TF::AQUA . "To " . $target->getCosmetics()->getNameColor() . $target->getRank()->rankString() . TF::GRAY . " » " . TF::WHITE . $msg);

    $target->getChatHandler()?->setLastMessagedPlayerName($sender->getName());
    $sender->getChatHandler()?->setLastMessagedPlayerName($target->getName());

    self::staffMessage($sender, $target, $msg);
  ...
  private static function staffMessage(SwimPlayer $sender, SwimPlayer $receiver, string $msg): void
  {
    ...
    $players = SwimCoreInstance::getInstance()->getServer()->getOnlinePlayers();
